...
let handle_test expected_l expected_r given =
  ...
  let eq e1 e2 =
    ...
    match e2 with
    | n, None ->
      if Float.compare coef1 n = 0 then
        Int.compare 0 pow1 = 0
      else
        false
    | n, Some (_, p) ->
      if Float.compare coef1 n = 0 then
        Int.compare pow1 p = 0
      else
        false
  in
...
let test_parser () =
   ...
   * TEST 22
   *)
  handle_test [ (0, 0.) ] [ (0, 0.) ] "0 = 0";

  (*
   * TEST 23
   *)
  handle_test [ (0, 0.) ] [ (0, 3.) ] "0 = 3";

  (*
   * TEST 24
   *)
  handle_test [ (1, -1.); (0, 1.) ] [ (0, 0.) ] "-x + 1 = 0";
