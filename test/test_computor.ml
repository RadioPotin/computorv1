...
let () =
  ...
  let s =
    Format.asprintf "%a" Solver.solve (Parser.equation Lexer.token (Lexing.from_string "5 * X^0 + 13 * X^1 + 3 * X^2 = 1 * X^0 + 1 * X^1")) in
  assert (String.equal "Reduced form: 4 * X^0 + 12 * X^1 + 3 * X^2 = 0\nPolynomial degree: 2\nDiscriminant is strictly positive, the two solutions are:\n-3.63299\n-0.367007\n" s);

  let s =
    Format.asprintf "%a" Solver.solve (Parser.equation Lexer.token (Lexing.from_string "5 * X^0 = 4 * X^0 + 7 * X^1" )) in
  assert (String.equal "Reduced form: 1 * X^0 - 7 * X^1 = 0\nPolynomial degree: 1\nThe solution is:\n0.142857\n" s);

  let s =
    Format.asprintf "%a" Solver.solve (Parser.equation Lexer.token (Lexing.from_string "4 * X^0 = 8" )) in
